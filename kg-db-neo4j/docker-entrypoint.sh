...
_download_dump(){
    local dump="$1"
    if [ ! -d "/data/databases/$dump.db" ]
    then
       echo "Database does not exist"
       echo "Downloading dump from http://ambiversenlu-download.mpi-inf.mpg.de/neo4j/$dump.tar.gz to ${DATA_PATH}..."

       wget -q --no-cookies -O - "http://ambiversenlu-download.mpi-inf.mpg.de/neo4j/$dump.tar.gz" \
...
if [ -d "${DATA_PATH}" ]; then
     if [ "$DUMP_NAME" ]; then
        _remove_data_tmp
        _download_dump "${DUMP_NAME}"

        python neo4_import_database.py
